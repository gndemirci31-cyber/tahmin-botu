...
ELO_HOME_ADV = 40
W_MKT_INIT = 0.45
HOME_ADV_CAP_PCT = float(os.getenv('HOME_ADV_CAP_PCT', '0.04'))  # Â±4pp cap around 0.5
...
def blend_model_market(model_probs, market_probs):
    """
    Model + Market karÄ±ÅŸÄ±mÄ± ve ev-bias tavanÄ± uygular.
    - model_probs: (pH, pD, pA)
    - market_probs: (pH, pD, pA) veya None
    - Cap: HOME_ADV_CAP_PCT (0.04 => 50% Â± 4pp)
    """
    def _normalize(p):
        s = sum(p)
        return tuple(x/max(s, 1e-9) for x in p)
    def _apply_home_cap(ph, pd, pa, cap):
        hi = 0.5 + cap
        lo = 0.5 - cap
        if ph > hi:
            excess = ph - hi
            ...
            if s > 0:
                ...
                pd = max(pd, 0.01)
                pa = max(pa, 0.01)
        return _normalize((ph, pd, pa))

    pH, pD, pA = model_probs
    if market_probs:
        w = float(get_w_mkt())
        pH = (1.0 - w) * pH + w * market_probs[0]
        pD = (1.0 - w) * pD + w * market_probs[1]
        pA = (1.0 - w) * pA + w * market_probs[2]
    ph, pd, pa = _normalize((pH, pD, pA))
    ph, pd, pa = _apply_home_cap(ph, pd, pa, float(HOME_ADV_CAP_PCT))
    return ph, pd, pa
