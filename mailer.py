# -*- coding: utf-8 -*-
"""
Tahmin Botu — tek parça mailer.py (LİG/KUPA FİLTRELİ)
(GÜNCEL: Elo + OppAdj Form + Hava + Odds + API-Football ipucu + High-Alert ayrı mail
 ...
 + Sadece belirtilen lig/kupaları listeleyen filtre)
...
def is_allowed_competition(area_name: str, comp_name: str) -> bool:
    ...
    # Doğrudan çift eşleşmesi
    if (a, c) in _ALLOWED_PAIRS:
        return True

    # Bazı yaygın varyantları yakala
    # Türkiye
    if a in ("turkey", "türkiye") and c in ("super lig", "süper lig", "1. lig"):
        return True
    # Almanya: 2nd tier eşleştirmeleri
...
def fetch_apifoot_fixtures(date_str):
    ...
    Sadece istenen lig/kupa filtresine uyan ve oynanmamış (NS/PST/TBD/SUSP/1H/HT/2H) maçları listeler.
    """
    if not APIFOOT:
        return []
...
# --- OpenLigaDB fallback (yalnızca Almanya alt ligleri için bırakıldı) -------
...
def fetch_odds_fixtures(date_str):
    ...
    for skey, area, comp in keys:
        ...
        for ev in data:
            ...
            if dtp.astimezone(TR_TZ).strftime("%Y-%m-%d") != date_str:
                continue
            if not is_allowed_competition(area, comp):
                continue
            out.append({
                "source": "ODDS",
                "utc_kickoff": dtp,
...
# --- Fikstür toplayıcı (filtre + fallback zinciri) ---------------------------
...
_API_LEAGUE_MAP = {
    ...
    "Turkey|1. Lig":          204,
    # Diğer ligler haritalı değilse ipucu/standings için sadece FD yolu kullanılır.
}
...
def report_predictions(date_str):
    ...
    lines = [f"⚽ Günün Tahminleri — {date_str} (Filtreli: sadece izinli lig/kupalar)\n"]
    top = []; hi = []
    ...
    if len(lines) == 1:
        lines.append("Filtreler nedeniyle listelenecek maç kalmadı (izinli lig yok/redundant) veya MIN_CONF yüksek olabilir.")
