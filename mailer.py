...
# --- 6C: Kart/Korner kayÄ±tlarÄ±ndan takÄ±m bazlÄ± ortalama Ã¼retimi ---
def _num_or_none(x):
    try:
...
def _aggregate_cards_corners_from_recs(home_team: str, away_team: str, recs: list[dict]):
    """recs: list[dict] (JSON/CSV kayÄ±tlarÄ±). DÃ¶ndÃ¼rÃ¼r: {"mu_cards_hint": x?, "mu_corners_hint": y?} veya None"""
    if not recs:
        return None
    ht = _norm_team(home_team); at = _norm_team(away_team)
    # candidate columns
    ...
    for r in recs:
        ...
        for k in home_keys:
            if k in d and d[k]:
                hv = _norm_team(d[k]); break
        for k in away_keys:
            if k in d and d[k]:
                av = _norm_team(d[k]); break
        if not hv or not av:
            continue
...
# 3. Fallback: KullanÄ±cÄ± URL listeleri (resmÃ® â†’ aÃ§Ä±k kaynak)
try:
    ...
    if merged and ("mu_cards_hint" in merged or "mu_corners_hint" in merged):
        log(f"Kart/Korner verisi URL fallback ile alÄ±ndÄ±: {home_team} vs {away_team}")
        return merged, "URL"
except Exception as e:
    log(f"URL kart/korner fallback hatasÄ±: {e}")

# 4. Fallback: Yerel snapshot
try:
    import json, csv, io
    # TekleÅŸmiÅŸ bir snapshot dosyasÄ± varsayalÄ±m; isim yoksa atla
    for path in ["data/cards_corners.json", "data/cards_corners.csv"]:
        ...
        if recs:
            ...
            if agg and ("mu_cards_hint" in agg or "mu_corners_hint" in agg):
                log(f"Kart/Korner verisi yerel snapshot ile alÄ±ndÄ±: {home_team} vs {away_team}")
                return agg, "LOCAL"
except Exception as e:
    log(f"Yerel kart/korner fallback hatasÄ±: {e}")
...
def report_predictions(date_str):
    ...
    if not fixtures:
        send_mail(f"GÃ¼nÃ¼n Tahminleri | {date_str}", "BugÃ¼n iÃ§in tahmin Ã§Ä±karÄ±lacak maÃ§ bulunamadÄ±.")
        return
...
def report_results(date_str):
    ...
    if not results:
        lines.append("BugÃ¼n iÃ§in sonuÃ§ bulunamadÄ±.")
